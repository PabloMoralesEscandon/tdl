...
int parse_options(int argc, char **argv, char **options) {
    ...
    while ((opt = getopt_long(argc, argv, "p:r:d:s:c:P:n:D:h", long_options,
                              NULL)) != -1) {
        switch (opt) {
        case 'p':
...
int cmd_add(char *options[], int id) {
    ...
    if (options[RECURRENT]) {
        if (!strcmp(options[RECURRENT], "daily"))
            new_task.recurrent = DAILY;
        else if (!strcmp(options[RECURRENT], "weekly"))
            new_task.recurrent = WEEKLY;
        else if (!strcmp(options[RECURRENT], "monthly"))
            new_task.recurrent = MONTHLY;
        else if (!strcmp(options[RECURRENT], "yearly"))
            new_task.recurrent = YEARLY;
        else {
    ...
    if (options[DUE]) {
        ...
        if ((strlen(options[DUE]) == 10) &&
            ...
            ((options[DUE][2] == options[DUE][5]) &&
             (options[DUE][2] == '-'))) {
            date.tm_mday =
                (options[DUE][0] - '0') * 10 + (options[DUE][1] - '0');
            date.tm_mon =
                (options[DUE][3] - '0') * 10 + (options[DUE][4] - '0') - 1;
            date.tm_year =
                (options[DUE][8] - '0') * 10 + (options[DUE][9] - '0');
            date.tm_year +=
                (options[DUE][6] - '0') * 1000 + (options[DUE][7] - '0') * 100;
            date.tm_year -= 1900;
        } else {
...
int cmd_mod(char *options[], int id) {
    ...
    if (options[PRIORITY]) {
        ...
        if (priority == -1) {
            ...
            to_do_list.items[index].priority = priority;
    }
    if (options[RECURRENT]) {
        if (!strcmp(options[RECURRENT], "daily"))
            to_do_list.items[index].recurrent = DAILY;
        else if (!strcmp(options[RECURRENT], "weekly"))
            to_do_list.items[index].recurrent = WEEKLY;
        else if (!strcmp(options[RECURRENT], "monthly"))
            to_do_list.items[index].recurrent = MONTHLY;
        else if (!strcmp(options[RECURRENT], "yearly"))
            to_do_list.items[index].recurrent = YEARLY;
        else {
            printf("Invalid recurrence option: %s\nValid options: daily, "
                   "weekly, monthly, yearly\n",
                   options[RECURRENT]);
